...
# When enabled, synthetic shoemarks are generated by the GAN (possibly on a
# separate GPU); otherwise grayscale shoeprints are used in their place
if config["gan"]["enabled"]:
    if config["gan"]["backend"] == "unsb":
        # UNSB is a flat script-style repo (not installable); import its
        # handler from the directory containing the referenced config.toml
        ...
        aug_device = torch.device(
            f"cuda:{gan_opt.gpu_ids[0]}"
            if torch.cuda.is_available() and gan_opt.gpu_ids
            else "cpu"
        )
        generator_handler = GeneratorHandler(gan_opt, aug_device)
    else:
        ...
        generator_handler = GeneratorHandler(gan_config, aug_device)
    if config["gan"]["compile"]:
        # Only the batch dimension varies at inference, so a dynamic-shape
        # compile avoids recompiling for every new batch size
        generator_handler.generator = torch.compile(  # pyright: ignore [reportAttributeAccessIssue]
            generator_handler.generator, dynamic=True
        )
    difficulty_scheduler = DifficultyScheduler(
        initial_difficulty=config["gan"]["initial_difficulty"],
        max_difficulty=config["gan"]["max_difficulty"],
        peak_steps=config["gan"]["peak_steps"],
    )
else:
    aug_device = device
    generator_handler = None
    # A pooled run can still follow the difficulty curriculum: the dataset
    # draws from difficulty-stratified pool trees (see StreamingDataset)
...
shoeprint_adaptive_norm = AdaptiveNormalisation(
    IMAGENET_MEAN, IMAGENET_STD, device=aug_device, momentum=0.9
)
...
# from disk and the augmentation pipeline substitutes them for in-loop generation
        ...
        for epoch in range(start_epoch, config["training"]["epochs"]):
            ...
            for _ in range(batches_per_epoch):
                ...
                with torch.no_grad():
                    shoeprints, shoemarks = augment_batch(
                        shoeprint_batch.to(aug_device),
                        shoeprint_gen_batch.to(aug_device),
                        floor_image_batch.to(aug_device),
                        shoemark_batch.to(aug_device),
                        shoemark_type_mask_batch.to(aug_device),
                    ...
                    # Transfer from the GAN GPU if augmentation happened there
                    if aug_device != device:
            ...
            if (
                ...
                checkpoint = {
                    ...
                    "shoeprint_scheduler_state_dict": shoeprint_scheduler.state_dict(),  # noqa: E501
